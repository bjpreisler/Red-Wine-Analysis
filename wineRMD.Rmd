...
p2 <-ggplot(wine, aes(x = citric.acid)) +
  geom_histogram(binwidth = .01)

p3 <- ggplot(wine, aes(x = residual.sugar)) +
  geom_histogram(binwidth = .1)

p4 <- ggplot(wine, aes(x = total.sulfur.dioxide)) +
  geom_histogram(binwidth = 5)

p5 <- ggplot(wine, aes(x = pH)) +
  geom_histogram(binwidth = .1)

p6 <- ggplot(wine, aes(x = sulphates)) +
  geom_histogram(binwidth = .1)

p7 <- ggplot(wine, aes(x = alcohol)) +
  geom_histogram(binwidth = .1)
...
The above correlation matrix allows us to very quickly spot variables that
either have a positive or negative correation to each other.  Another type of
visualization to see how variables interact is a plot matrix.
Let's take a look at the inter-correlations below:
...
regards to determing the quality of a red wine.
...
fact that alcohol is positively correlated with quality and volatile acidity
is negatively correlated to quality is concisely shown in one graph.
